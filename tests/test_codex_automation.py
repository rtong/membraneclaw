...
class CodexAutomationTests(unittest.TestCase):
    ...
    def test_usage_limit_stops_launching_the_remaining_batch(self):
        calls = []
        tasks = []
        for index in range(10):
            task = _teacher_task()
            task["task_id"] = f"teacher::q{index}"
            task["case_id"] = f"q{index}"
        ...
        def exhausted_invoker(request, _root, _timeout):
        ...
        with tempfile.TemporaryDirectory() as tmp, patch.dict(os.environ, {}, clear=True):
            with self.assertRaisesRegex(RuntimeError, "usage limit"):
                run_codex_tasks(
                    ...
                    concurrency=2,
                    retries=0,
                    invoker=exhausted_invoker,
                )

        self.assertLessEqual(len(calls), 2)
        self.assertLess(len(calls), len(tasks))
