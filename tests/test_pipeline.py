...
from auto_evaluate.trajectory_metrics import build_trajectory_metrics  # noqa: E402
...
class PipelineTests(unittest.TestCase):
    def test_d6_state_metrics_respect_explicit_state_lineage_rubric_coverage(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_json(
                ...
                {
                    "mapping": [
                        {"task_id": "judge::D6-6a-n01::tools", "case_id": "D6-6a-n01", "system_id": "tools"},
                        {"task_id": "judge::D6-6b-legacy::tools", "case_id": "D6-6b-legacy", "system_id": "tools"},
                    ]
                },
            )
            write_json(
                run_dir / "benchmarks" / "D6-6a-n01.json",
                {
                    "case_id": "D6-6a-n01",
                    "task_family": "d6_6a",
                    "tool_efficiency_rubric": {
                        "dimensions": [
                            {"dimension_id": "E3", "dimension_label": "E3 State lineage and adaptive iteration"}
                        ]
                    },
                },
            )
            write_json(
                run_dir / "benchmarks" / "D6-6b-legacy.json",
                {
                    "case_id": "D6-6b-legacy",
                    "task_family": "d6_6b",
                    "tool_efficiency_rubric": {
                        "dimensions": [
                            {"dimension_id": "E3", "dimension_label": "E3 Information gain"}
                        ]
                    },
                },
            )
            for case_id in ("D6-6a-n01", "D6-6b-legacy"):
                write_json(
            ...
            base_rating = {
                "total_score": 50,
                "steps": [{"failure_codes": ["TOOL_ARGUMENT"]}],
                "tool_efficiency_dimensions": [{"dimension_id": "E3", "score": 8, "max_score": 20}],
                "trajectory_analysis": {"path_classification": "valid_alternative", "first_error_event_id": "T002"},
                "causal_analysis": {"first_error_step_id": 3, "downstream_affected_steps": [3, 4], "root_cause": "state lost"},
            }
            write_jsonl(
                run_dir / "ratings.jsonl",
                [
                    {"task_id": "judge::D6-6a-n01::tools", "case_id": "D6-6a-n01", **base_rating},
                    {"task_id": "judge::D6-6b-legacy::tools", "case_id": "D6-6b-legacy", **base_rating},
                ],
            )

            state = build_trajectory_metrics(run_dir)["d6_state"]
            self.assertEqual(2, state["d6_case_count"])
            self.assertEqual(1, state["state_lineage_case_coverage"])
            self.assertEqual(2, state["groups"]["D6-all"]["tools"]["n"])
            self.assertEqual(1, state["groups"]["D6-all"]["tools"]["state_lineage_n"])
            self.assertEqual(8.0, state["groups"]["D6-all"]["tools"]["mean_state_lineage_score"])
            self.assertEqual(1.0, state["failures"]["D6-all"]["tools"]["TOOL_ARGUMENT"]["incidence"])
    ...
    def test_execute_run_honors_system_generation_overrides(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(["system", "system"], client.prompts)
    ...
    def test_execute_run_archives_failed_response_before_collection_resume(self):
        with tempfile.TemporaryDirectory() as tmp:
            temp = Path(tmp)
            run_dir = temp / "runs" / "attempt-history"
            benchmark = {
                "case_id": "q-history",
                "question_prompt": "question",
                "source": {"sha256": "case-sha"},
            ...
            self.assertEqual({"success": 0, "error": 1, "skipped": 0}, first)
            self.assertEqual({"success": 1, "error": 0, "skipped": 0}, second)
            archived = read_json(
                run_dir
                / "response_attempts"
                / "q-history__baseline"
            ...
            self.assertEqual("error", archived["response"]["status"])
            self.assertEqual("incomplete_response", archived["response"]["error_type"])
            final = read_json(run_dir / "responses" / "q-history__baseline.json")
            self.assertEqual("success", final["status"])
            self.assertEqual(2, final["collection_attempt"])

    def test_execute_run_stops_at_collection_attempt_cap(self):
        with tempfile.TemporaryDirectory() as tmp:
            temp = Path(tmp)
            run_dir = temp / "runs" / "attempt-cap"
            benchmark = {
                "case_id": "q-cap",
                "question_prompt": "question",
                "source": {"sha256": "case-sha"},
            ...
            self.assertEqual({"success": 0, "error": 1, "skipped": 0}, first)
            self.assertEqual({"success": 0, "error": 1, "skipped": 0}, second)
            self.assertEqual({"success": 0, "error": 1, "skipped": 0}, third)
            self.assertEqual(2, client.calls)
            final = read_json(run_dir / "responses" / "q-cap__tools.json")
            self.assertEqual(2, final["collection_attempt"])
...
Final answer'''
    ...
    def test_required_tool_contract_rejects_irrelevant_successful_tools(self):
        from auto_evaluate.trajectory import extract_observable_trajectory

        generic_tools = extract_observable_trajectory(
            '`🔧 calculator({"expression":"58"})`\n'
            '`↳ 58`\nFinal answer',
            tools_enabled=True,
        )
        error = required_tool_call_error(
            generic_tools,
            required=True,
            required_tool_names=["simulate_ro", "simulate_swro_system"],
        )
        self.assertIn("simulate_ro", error)
        self.assertIn("simulate_swro_system", error)
        self.assertIn("calculator", error)

        required_tools = extract_observable_trajectory(
            '`🔧 ro-chem-simulate_ro({"area":70})`\n'
            '`↳ {"flux":15.7}`\n'
            '`🔧 ro-chem-simulate_swro_system({"ro_membrane_area_m2":7000})`\n'
            '`↳ {"sec_kwh_m3":2.6}`\nFinal answer',
            tools_enabled=True,
        )
        self.assertIsNone(
    ...
    def test_probe_chat_resolves_only_the_requested_model(self):
        cases = [(["--system", "tools"], {"OPENWEBUI_MODEL_TOOLS": "tools"}, "tools"),
                 (["--model", "direct-9b"], {}, "direct-9b")]
        for flags, environment, expected in cases:
            with self.subTest(flags=flags), patch.dict("os.environ", environment, clear=True), \
                 patch("auto_evaluate.cli.load_dotenv"), patch("auto_evaluate.cli.make_client") as make_client, \
                 patch("builtins.print"):
                args = cli.build_parser().parse_args(["probe-chat", "--stream", *flags])
                make_client.return_value.chat_stream.return_value = argparse.Namespace(content="OK", latency_ms=1)
                self.assertEqual(0, cli.command_probe_chat(args))
                self.assertEqual(expected, make_client.return_value.chat_stream.call_args.kwargs["model"])
                make_client.return_value.chat_stream.assert_called_once()

    def test_probe_chat_still_requires_selected_system_credentials(self):
        args = cli.build_parser().parse_args(["probe-chat", "--system", "tools"])
        with patch.dict("os.environ", {}, clear=True), patch("auto_evaluate.cli.load_dotenv"), \
             patch("auto_evaluate.cli.make_client") as make_client:
            with self.assertRaisesRegex(ValueError, "Missing model ID environment variable: OPENWEBUI_MODEL_TOOLS$"):
                cli.command_probe_chat(args)
            make_client.assert_not_called()

    def test_normal_system_loading_still_requires_recovery_model(self):
        with patch.dict("os.environ", {"OPENWEBUI_MODEL_TOOLS": "tools"}, clear=True):
            with self.assertRaisesRegex(ValueError, "context_recovery: missing model ID environment variable"):
                cli.load_systems(ROOT / "configs" / "systems.json", ["tools"])
    ...
    def test_teacher_judge_validation_and_report(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((run_dir / "figures" / "tools-family-gain.svg").exists())
            self.assertTrue((run_dir / "figures" / "tools-step-gain.svg").exists())
            self.assertTrue((run_dir / "figures" / "quality-efficiency.svg").exists())
            self.assertTrue((run_dir / "figures" / "rag-paired-outcomes.svg").exists())
            self.assertTrue((run_dir / "figures" / "rag-family-effect.svg").exists())
            self.assertTrue((run_dir / "figures" / "trajectory-first-error.svg").exists())
            self.assertTrue((run_dir / "figures" / "trajectory-failure-incidence.svg").exists())
            self.assertTrue((run_dir / "figures" / "trajectory-efficiency-dimensions.svg").exists())
            self.assertTrue((run_dir / "figures" / "trajectory-recovery-quality.svg").exists())
            self.assertTrue((run_dir / "figures" / "d6-state-lineage.svg").exists())
            self.assertTrue((run_dir / "figures" / "d6-state-failures.svg").exists())
