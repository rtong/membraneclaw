...
# loss on the value of `action` in the answer turn of the gold tool episode and nothing else. root_cause is
# not supervised: on the tool line organic_fouling is already emitted. lr 1e-4 as in 10/11, no dead weight.
# Doses: epoch 1, 2, 3 of one pass; dose 0 is runs/tool-sft-raw itself.
#
# Probe per dose: 150 train cases x 4 samples at temperature 1.0 (10's probe), plus greedy dev.
# Choice, fixed before any of it runs: the smallest dose that
#   (a) emits the severe action at least once in the 600 samples,
#   (b) has not collapsed the slot: severe said on at most 10% of the samples whose answer is not severe,
#   (c) keeps the calls: greedy dev numeric_acc >= 0.99.
cd "$(dirname "$0")/../.."
...
$PY ppo_ac.py --prompt-tool 1 --sft-only 1 --sft-tool 1 --sft-seed-from runs/tool-sft-raw/adapter \
    ...
    --sft-epochs 3 --sft-lr 1e-4 --max-vram-mib 13000 --out $SEED > $OUT/sft.log 2>&1
echo "sft exit $?" >> $OUT/status

probe() {  # name, run directory
    $PY ppo_ac.py --prompt-tool 1 --eval-only 1 --resume-from "$2" --eval-split train --eval-cases 150 \
        --eval-samples 4 --eval-temperature 1.0 --max-vram-mib 13000 \
...
probe epoch0 runs/tool-sft-raw
for k in 1 2 3; do
    probe epoch$k $SEED/epoch$k
    dev epoch$k $SEED/epoch$k
done
