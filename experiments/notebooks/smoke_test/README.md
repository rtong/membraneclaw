...
| `09_one_value_the_policy_would_not_say.ipynb` | the policy emits `flat` **zero times in 600 flag slots** and it is the right answer in 184 of them — `0.959 x 416/600 = 0.665` is `flags_acc` exactly. It is a prior and not a capability: non-flat slots within 2-5 pp of a threshold are called correctly 37/38. Prompting barely moves it (26/184 when told outright), an 8x credit recovers it and destroys everything else, targeted entropy is the right diagnosis and the wrong tool. **3x credit for a correct `flat`** takes `flat` recall 0.000 -> 0.984, all-three-flags 0.215 -> 0.780 and `exact_match` 0.145 -> **0.405**, with the arithmetic exact throughout and the critic's residual over a clock at its series best. Also records that two runs of an identical command diverge at step 3. Five runs. |
| `12_a_calculator_the_model_has_to_call.ipynb` | **withdraws `08`'s v3.** The prompt is v2's, byte for byte, and the arithmetic goes back to the model with a calculator it has to call (Qwen3's `<tool_call>` protocol). A seed from the **raw** model supervises only the three calls: 600/600 held-out calls match the gold expression and `numeric_acc` goes 0.067 -> **0.997**, and stays 0.993-1.000 held out through PPO. Two 40-step gate runs: the first failed its critic gate because the value head was warm-started at v2's 0.2615 (fixed: `FROZEN_BASELINE_TOOL`, `--value-head-reset`); the corrected one puts the critic in `09`'s band but fails the per-step arithmetic gate once (a sampled episode ran out of budget). Neither converged; `exact_match` 0.160. |
| `13_an_agent_harness_and_where_the_entropy_goes.ipynb` | `12`'s line run to 200 steps peaks at `exact_match` **0.600** (step 125) and then **collapses** -- the entropy bonus, averaged over episodes that are 72% calls, drives the calls off their seed. **13a**, the numeric credit paid on the calls with a settlement at the copy, is abandoned after three runs (repeated calls, two collapses). **13b** moves the loop under qwen-agent (`--harness qwen_agent`, equivalent to the native harness on all 200 dev cases, episode for episode) and scopes the bonus to the answer turn: 200 steps **without collapse**, `exact_match` **0.555** and rising, held-out `numeric_acc` 0.985-0.997, critic ahead of the clock on 50/50 steps from 150. One seed. |
...
* `12` -- executed, committed with outputs. **`08`'s v3 is withdrawn**: printing the three answers into
  the prompt changed the task, and every `numeric_acc = 1.000` from `08` to `11` was the harness's. The
  prompt is now v2's, unchanged (checked on all 850 cases; the only added text is the tool declaration
  the chat template inserts), and the model computes through a calculator tool. A seed from the raw
  model, with the loss on the three calls only, takes held-out `numeric_acc` 0.067 -> 0.997 with
  600/600 calls identical to the gold expression. The first 40-step PPO gate run failed its critic gate
  because of a mis-set warm start (0.2615, v2's, instead of the seed's 0.6521) -- not, as a first draft
  said, because of the episode's shape. Corrected, the critic lands where `09`'s own command lands on
  today's code (-0.0072 vs -0.0006 over steps 20-39; "median > 0" turns out to sit on the noise floor),
  and the per-step arithmetic gate fails once (0.833 at step 11). Not converged (`exact_match` 0.160 at
  step 40), not extended. `10`'s dead labels are deferred; the severe action is still 0/37. Next is `13`:
  numeric credit on the calls, then a per-turn discount, then the Qwen-Agent harness.

* `13` -- executed, committed with outputs. `12`'s corrected configuration run to 200 steps peaks at
  `exact_match` 0.600 at step 125 and collapses (all-zero at 200): entropy leaves its band at ~100,
  sampled arithmetic fails from 128, tokens run away from 160. `13a` (credit on the calls) is
  abandoned: `lam` 0.95 collapses, `lam` 1.0 doubles the calls and scores 0.205 at step 50, a critic
  warm-up collapses 14 steps after it ends. `13b` runs the loop under qwen-agent -- the framework
  `membraneclaw-agent` uses, imported read-only, reproducing `12`'s G2 case for case -- with the
  entropy bonus off the call tokens: 200 steps, no collapse, `exact_match` 0.555, `cause_acc` 0.710,
  held-out `numeric_acc` never below 0.985. Gates missed as set: three single steps of sampled
  arithmetic at 0.875, and the critic's early window (steps 20-39). Open: a second seed, a per-turn
  discount, past 200 steps, and the severe action (still never emitted; caps `exact_match` at 0.815).
