...
| `14_the_label_ppo_would_not_keep.ipynb` | The severe action, still said **zero times in 37** at the end of `13`, is seeded from the raw model -- the tool seed's own examples and 150 of `11`'s off-distribution records in one supervised pass, with the loss on the calls, the answer's opening token and the label values. Seeding it is not enough: from a seed that is right on 27% of the times it says the label, PPO **deletes** it (26 emissions -> 5), while `organic_fouling`, seeded at one correct emission in 600 and right when said, is **amplified to 29 of 29**. Precision at the seed, not rarity, decides which. A per-token `gamma = 0.999` is the only discount whose run reached 200 steps (**0.635**, against 0.555/0.560 for `13b`), and **3x credit for a correct severe action** -- `09`'s lever on the action slot -- takes the label to **37 of 37** and `exact_match` to **0.855 dev / 0.825 test / 0.740 holdout_shift**. |
