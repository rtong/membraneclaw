...
- do not spend search budget refining an already-passing or non-limiting case beyond one verification;
- keep a checklist of which case is resolved and which one still binds, and move budget to the
  case that binds.
...
- a transcript that ends mid-reasoning with no conclusion is a failure, no matter how much
