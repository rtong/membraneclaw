...
| Agent process is up and `systemctl` says `active`, but `/health` times out and nothing is served | An MCP tool call that never came back. qwen-agent waits on it with a bare `future.result()` (`mcp_manager.py:280`) — no timeout, ever. What turned one stuck call into a dead server: `chat_completions` is `async def` and iterated the *synchronous* `_run_agent` inline, so the block sat on the asyncio event loop. Measured once: 2h40m of silence, `py-spy dump` showing `mcp_manager.py:280` under `run (asyncio/runners.py)`, while the MCP server's own log had the same call returning `200 OK` — the reply was lost on the streamable-http session, not slow to compute | `MCP_CALL_TIMEOUT` (default 300s, `agent/compat.py`) bounds the wait; `run_in_threadpool` in `server.py` keeps the non-streaming path off the event loop. Diagnose with `.venv/bin/py-spy dump --pid $(systemctl show membraneclaw-agent -p MainPID --value)` — a hung agent and a slow one look identical from outside |
