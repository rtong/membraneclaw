...
- 当前正在开发什么：[CURRENT_EXPERIMENT.md](CURRENT_EXPERIMENT.md)
- 实验目录结构说明：[experiments/README.md](experiments/README.md)
- 全部实验状态：[experiments/REGISTRY.md](experiments/REGISTRY.md)
- Skill角色与版本：[skills/README.md](skills/README.md)

功能代码保留在 `src/`，实验配置保留在 `configs/`，版本化Skill保留在 `skills/`，原始结果
保留在 `runs/`；`experiments/` 只负责把一次实验涉及的文件和结果串联起来，不复制实现。
...
Tools Teacher 通过无交互 CLI 使用当前 ChatGPT 账号已连接并授权的 WaterTAP connected
app；运行时调用来源显示为 `codex_apps`。项目不注册独立的 `watertap` MCP，也不读取
`WATERTAP_MCP_TOKEN`；不要在 `.env` 中添加该变量，也不要运行 `codex mcp add watertap`。
旧设备如果曾经添加过独立配置，只需用项目安装的 bundled CLI 执行一次下面的命令；账号级
connected app 不会因此被删除，而且通常不会出现在本地 `mcp list` 中：

```bat
python -c "from codex_cli_bin import bundled_codex_path; import subprocess; raise SystemExit(subprocess.call([str(bundled_codex_path()), 'mcp', 'remove', 'watertap']))"
```

开始评测前，应在 Codex 或 ChatGPT 客户端确认当前账号的 WaterTAP connected app 已连接且
可以正常调用。Tools Teacher 默认使用 `codex exec --approve-for-me`，避免后台进程因无法点击
工具确认而产生 `user cancelled MCP tool call`。代码同时要求至少一个 WaterTAP 调用真正成功
且返回非空 observation；仅出现工具事件但调用失败不会进入评分。
...
```bat
python skills\_historical\swro-watertap\v0.8.9\build_skill.py
```
...
## 教师蒸馏 Solver Skill 小样本（独立于旧实验）

离线合同执行器仍通过以下命令验证；其中 `completed` 不代表模型涨分：

```bat
python -m unittest discover -s tests -p test_skill_runtime.py -q
python skill_runtime.py run-fixture --task examples/executable_skills/parallel_task.json --skill skills/swro-parallel-compare/v0.1.1 --fixture examples/executable_skills/parallel_fixture.json --out runs/skill-runtime-demo-NEW
```

输出目录须为新目录。真实9B的第一阶段只比较C00（Tools）与C10（相同Tools加本地冻结短
Skill），不测试硬执行器或自动进化。P3每条件获取三份满足目标WaterTAP工具门禁的有效回答，
主指标取各条件最高Judge分（best-of-3）。当前等待远程WaterTAP工具绑定恢复；恢复后使用新的
v3 run，不继续评分无效的v1/v2：

```powershell
python ae.py solver-skill-pilot --run-id solver-skill-p3-c00-c10-v3 --stage prepare
python ae.py solver-skill-pilot --run-id solver-skill-p3-c00-c10-v3 --stage systems --system-concurrency 2
...
也可用 `--stage all` 创建新run并连续执行。成功响应按请求哈希复用；失败episode最多采集四次，
每次替换前归档。P3必须同时观察到 `simulate_ro` 和 `simulate_swro_system` 的成功结果；
`calculator` 或 `http_get` 不算有效完成。P1需要单独的边界搜索Skill，不能错误复用并联Skill。
完整规则见 `docs/SOLVER_SKILL_PILOT_PROTOCOL.md`，结构化入口见
`experiments/30-solver-skill-p3/README.md`。C00/C10出现有用信号后才实现C01/C11与离线晋升循环。

## Evaluation Skill Judge校准（独立固定回答）

该实验不重新运行9B，也不覆盖冻结评分。它把12份固定回答交给当前Judge（J0）和加入
`swro-evaluation-judge@0.1.0`的Judge（J1），每个条件独立重复两次。先准备不可变快照：

```bat
python ae.py judge-calibration --run-id judge-calibration-12-v1 --stage prepare
```

随后运行48个Judge任务并生成初步稳定性报告：
...
没有完整人工评分时只能比较重复稳定性和评分偏移，不能声称J1更准确。完整设计与晋升门槛见
`docs/JUDGE_SKILL_CALIBRATION_PROTOCOL.md`。
...
Codex Teacher/Judge 只维持不超过指定并发数的在途任务，不再一次性提交整个批次。首次检测到
usage-limit 或认证失效后会停止补充任务并取消尚未开始的工作；按 `Ctrl+C` 时也会保留已完成
记录并取消队列。因此终止后最多只需等待当时已经运行的少量请求退出，随后使用相同 run ID
即可从成功缓存继续。
