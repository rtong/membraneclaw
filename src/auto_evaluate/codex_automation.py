...
def _is_batch_fatal_error(exc: BaseException) -> bool:
    """Stop launching fresh Codex tasks when the shared service cannot accept them."""
    ...
    if pending:
        ...
        def submit_one() -> bool:
            ...
            try:
                ...
                return False
            future_map[executor.submit(execute, item)] = item[0]["task_id"]
            submitted += 1
            return True
        ...
        interrupted = False
        try:
            while future_map and not fatal_error:
                ...
                for future in completed:
                    ...
                    try:
                        ...
                        fatal_error = fatal_error or _is_batch_fatal_error(exc)
                while not fatal_error and len(future_map) < concurrency and submit_one():
                    pass
        except KeyboardInterrupt:
            interrupted = True
            cancelled = sum(future.cancel() for future in future_map)
            not_submitted = len(pending) - submitted
            print(
                f"[codex:{stage}] interrupted; cancelled {cancelled} queued task(s), "
                f"left {not_submitted} task(s) unsubmitted. Successful records remain cached.",
                file=sys.stderr,
            )
            ...
            if not interrupted:
                if fatal_error:
                    ...
                    not_submitted = len(pending) - submitted
                    print(
                        f"[codex:{stage}] stopping batch after a non-recoverable service error; "
                        f"cancelled {cancelled} queued task(s), left {not_submitted} task(s) unsubmitted. "
                        "Re-run the same command after service access is restored.",
                        file=sys.stderr,
                    )
