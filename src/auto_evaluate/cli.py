...
from .judge_calibration import (
    ...
    prepare_calibration,
    run_calibration_judges,
    write_calibration_analysis,
)
from .report import build_report
from .repeatability import build_repeatability_analysis, write_repeatability_analysis
...
from .solver_skill_pilot import (
    build_solver_skill_analysis,
    prepare_solver_skill_pilot,
    run_solver_skill_judges,
    run_solver_skill_systems,
    ...
    def add(
        ...
        *,
        ...
        expected_skill = (
            ...
            else remote_skill_id or (skill_version or "").split("@", 1)[0] or None
        )
...
def command_probe_chat(args) -> int:
    ...
    load_dotenv(root / ".env")
    _, systems_path, _ = _paths(args)
    config = load_systems(systems_path, [] if args.model else [args.system], load_recovery=False)
        ...
        if getattr(args, "require_complete_systems", False) and stage in {"all", "systems", "teachers", "judges"}:
            ...
            if completeness["incomplete"]:
                examples = "; ".join(
                    ...
                    + (
                        f"[{format_execution_error_for_log(row['error_type'])}]"
...
def command_judge_calibration(args) -> int:
    root = _root()
    run_dir = (root / "runs" / args.run_id).resolve()
    config_path = (root / args.config).resolve()
    if args.stage == "prepare":
        ...
        return 0
    if args.stage == "judges":
        load_dotenv(root / ".env")
        outputs = run_calibration_judges(
            root=root,
            run_id=args.run_id,
    ...
    errors = validate_ratings(run_dir)
    if errors:
        raise ValueError("Calibration ratings are incomplete or invalid: " + "; ".join(errors[:5]))
    analysis = build_calibration_analysis(run_dir)
    output = write_calibration_analysis(run_dir, analysis)
    print(
        json.dumps(
            {
                "status": "complete",
                ...
                "run_id": args.run_id,
                "output": str(output),
                "expert_status": analysis["expert_calibration"]["status"],
            },
            ensure_ascii=False,
            indent=2,
        )
...
def command_solver_skill_pilot(args) -> int:
    root = _root()
    run_dir = (root / "runs" / args.run_id).resolve()
    config_path = (root / args.config).resolve()
    stage = args.stage
    ...
    if stage == "all" and not (run_dir / "pilot_manifest.json").is_file():
        manifest = prepare_solver_skill_pilot(
            root=root,
            config_path=config_path,
            run_id=args.run_id,
        ...
        print(
            json.dumps(
                {"status": "prepared", "run_id": args.run_id,
                 "expected_solver_episodes": manifest["expected_solver_episodes"]},
                ensure_ascii=False,
                indent=2,
            )
        )
    if not (run_dir / "pilot_manifest.json").is_file():
        raise FileNotFoundError(
            f"Prepared Solver Skill pilot not found: {run_dir}; run --stage prepare first"
        )
    ...
    if stage in {"all", "systems"}:
        ...
        probe_args = argparse.Namespace(
            systems=str(run_dir / "systems.json"),
            details=True,
            selected_system_ids=pilot["system_ids"],
            benchmark_set=None,
            evaluation_profile="solver_skill_pilot_c00_c10",
        )
        _run_auto_step("probe shared Tools preset", command_probe, probe_args)
        system_result = run_solver_skill_systems(
            root=root,
            run_id=args.run_id,
            concurrency=args.system_concurrency,
        )
        print(json.dumps({"stage": "systems", **system_result}, ensure_ascii=False, indent=2))
        completeness = system_result["completeness"]
        if completeness["incomplete"]:
            return 1

    if stage in {"all", "judges"}:
        load_dotenv(root / ".env")
        ratings = run_solver_skill_judges(
            root=root,
            run_id=args.run_id,
            model=args.codex_model,
            concurrency=args.judge_concurrency,
        ...
        print(
            json.dumps(
                {"status": "complete", "stage": "judges", "ratings": len(ratings)},
                ensure_ascii=False,
                indent=2,
            )
        )
    ...
    if stage in {"all", "report"}:
        ...
        if errors:
            raise ValueError(
                "Solver Skill pilot ratings are incomplete or invalid: "
                + "; ".join(errors[:5])
            )
...
def command_repeatability_analysis(args) -> int:
    root = _root()
    selection_path = (root / args.selection).resolve()
    selection = read_json(selection_path)
    run_dirs = [(root / "runs" / run_id).resolve() for run_id in args.run_id]
    missing = [str(path) for path in run_dirs if not path.exists()]
    if missing:
        raise FileNotFoundError(f"Repeatability run directory not found: {missing[0]}")
    ...
    write_repeatability_analysis(output_dir, analysis)
    print(
        json.dumps(
            {
                "status": "complete",
                ...
                "n_replicates": analysis["n_replicates"],
                "comparisons": {
                    key: {
                        "mean_effect": value["mean_case_averaged_effect"],
                        "ci95": value["case_bootstrap_ci95"],
                        "run_effect_sd": value["run_effect_sd"],
                    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_solver_skill = sub.add_parser(
        "solver-skill-pilot",
        help="run the development-only C00/C10 teacher-distilled Solver Skill pilot",
    )
    p_solver_skill.add_argument("--run-id", required=True)
    p_solver_skill.add_argument(
        "--config", default="configs/solver_skill_pilot.json"
    )
    p_solver_skill.add_argument(
        "--stage", choices=("prepare", "systems", "judges", "report", "all"), required=True
    )
    p_solver_skill.add_argument("--system-concurrency", type=int, default=2)
    p_solver_skill.add_argument("--codex-model", default="gpt-5.6-sol")
    p_solver_skill.add_argument("--judge-concurrency", type=int, default=4)
    p_solver_skill.add_argument("--codex-retries", type=int, default=2)
    p_solver_skill.add_argument("--codex-timeout", type=int, default=3600)
    p_solver_skill.add_argument("--seed", type=int, default=20260907)
    p_solver_skill.add_argument("--force-codex", action="store_true")
    p_solver_skill.set_defaults(func=command_solver_skill_pilot)
    ...
    p_judge_calibration = sub.add_parser(
        "judge-calibration",
        help="compare the current Judge with a frozen Evaluation Skill on fixed answers",
    )
    p_judge_calibration.add_argument("--run-id", required=True)
    p_judge_calibration.add_argument(
        "--config", default="configs/judge_calibration_12.json"
    )
    p_judge_calibration.add_argument(
        "--stage", choices=("prepare", "judges", "report"), required=True
    )
    p_judge_calibration.add_argument("--codex-model", default="gpt-5.6-sol")
    p_judge_calibration.add_argument("--judge-concurrency", type=int, default=4)
    p_judge_calibration.add_argument("--codex-retries", type=int, default=2)
    p_judge_calibration.add_argument("--codex-timeout", type=int, default=3600)
    p_judge_calibration.add_argument("--force-codex", action="store_true")
    p_judge_calibration.set_defaults(func=command_judge_calibration)
    ...
    p_repeatability = sub.add_parser(
        "repeatability-analysis",
        help="aggregate repeated D1-D6 runs without treating replicates as independent cases",
    )
    p_repeatability.add_argument(
        "--run-id",
        action="append",
        required=True,
        help="run ID to include; repeat this option for every replicate",
    )
    p_repeatability.add_argument(
        "--selection",
        default="configs/d1_d6_repeatability_26.json",
    )
    p_repeatability.add_argument("--output-dir", default=None)
    p_repeatability.add_argument("--bootstrap-samples", type=int, default=5000)
    p_repeatability.add_argument("--seed", type=int, default=20260904)
    p_repeatability.set_defaults(func=command_repeatability_analysis)
