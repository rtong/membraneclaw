...
def load_systems(config_path: Path) -> dict[str, Any]:
    ...
    if [item.get("id") for item in systems] != ["agent", "agent-rag", "agent-rag-skill"]:
        raise ValueError("systems.json must define agent, agent-rag, agent-rag-skill in that order")
        ...
        for system in config["systems"]:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    result = chat_method(
                        model=system["model_id"], messages=messages, generation=generation
                    )
                    write_json(
                        path,
                        {
