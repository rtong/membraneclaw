...
def build_report(run_dir: Path, output_path: Path | None = None) -> Path:
    ...
    if not mean_cards:
        mean_cards.append('<div class="notice">评分尚未导入。完成 <code>ratings.jsonl</code> 后重新生成报告。</div>')
...
<div class="muted">Agent、Agent-RAG 与 Agent-RAG-Skill 的分步诊断报告</div>
