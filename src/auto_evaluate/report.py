...
from .trajectory_metrics import build_trajectory_metrics, write_trajectory_paper_package
...
COLORS = {
    ...
    "tools": "#0f766e",
    "tools-rag": "#0891b2",
    "tools-rag-skill-control": "#64748b",
    "tools-rag-solver-v060": "#d97706",
    "tools-rag-solver-v089": "#2563eb",
...
def build_report(run_dir: Path, output_path: Path | None = None) -> Path:
    ...
    if any(
        int(((trajectory_metrics.get("systems") or {}).get(system_id) or {}).get("n") or 0)
        for system_id in ("baseline", "tools", "tools-rag")
    ):
        write_trajectory_paper_package(run_dir)
    ...
    rag_counterfactual_section = (
        ...
        f'<div class="paper-figure">{figure_svgs["rag-paired-outcomes"]}</div>'
        f'<div class="paper-figure">{figure_svgs["rag-family-effect"]}</div>'
    ...
    trajectory_figure_section = (
        '<section class="panel"><div class="section-kicker">Trajectory experiment</div>'
        '<h2>执行轨迹与错误传播</h2>'
        '<p class="muted">该组图只复用可观察执行记录和既有 Judge 诊断。它描述错误首次出现、传播与执行效率，不构成严格的因果干预证明；原生完成和 context-reset 恢复完成分开报告。</p>'
        f'<div class="paper-figure">{figure_svgs["trajectory-first-error"]}</div>'
        f'<div class="paper-figure">{figure_svgs["trajectory-failure-incidence"]}</div>'
        f'<div class="paper-figure">{figure_svgs["trajectory-efficiency-dimensions"]}</div>'
        f'<div class="paper-figure">{figure_svgs["trajectory-recovery-quality"]}</div></section>'
    )
    d6_state = trajectory_metrics.get("d6_state") or {}
    d6_state_figure_section = (
        '<section class="panel"><div class="section-kicker">D6 State diagnosis</div>'
        '<h2>D6任务内State保持与证据血缘</h2>'
        '<p class="muted">State指同一道题内固定参数、候选编号、跨工具结果、约束状态和证据来源能否持续保持。'
        '该分析复用冻结轨迹，不是新增Tools+State系统，也不评价跨任务长期Memory。'
        'D6子组同时对应不同任务族和依赖结构，分层差异仅作描述，不能解释为严格的拓扑因果效应。</p>'
        f'<div class="paper-figure">{figure_svgs["d6-state-lineage"]}</div>'
        f'<div class="paper-figure">{figure_svgs["d6-state-failures"]}</div>'
        '<p class="muted">完整分层表、逐题root cause和Rubric覆盖说明已写入 '
        '<code>paper_stage1_trajectory/d6_state_*.csv</code> 与 '
        '<code>paper_stage1_trajectory/D6_STATE_RESULTS_CN.md</code>。</p></section>'
        if d6_state.get("d6_case_count") else ""
    )
...
<div class="meta"><span>生成时间：{_e(generated)}</span><span>Benchmark：{len(case_ids)}</span><span>待测系统最终成功：{success_count}/{len(responses)}</span><span>原生失败：{native_error_count}</span><span>上下文恢复：{recovered_count}</span><span>最终失败：{error_count}</span><span>Teacher 参考：{len(teacher_records)}</span><span>有效评分：{len(ratings)}</span></div></section>
{warning_block}
...
<section class="panel"><div class="section-kicker">Primary systems</div><h2>主要待测系统</h2><p class="muted">这里只展示论文的主要求解系统；强制RAG属于反事实分支，Adaptive属于路由策略，二者不再混入同一个模型排行榜。</p><div class="grid">{"".join(mean_cards)}</div><div class="paper-figure">{figure_svgs["main-scores"]}</div><div class="paper-figure">{figure_svgs["tools-family-gain"]}</div><div class="paper-figure">{figure_svgs["tools-step-gain"]}</div></section>
